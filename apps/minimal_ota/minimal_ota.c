...
#define UPDATE_FILENAME  "FLASH0:why2025_firmware.bin"
...
void read_update_file(app_state_t *app) {
    FILE *f = fopen(UPDATE_FILENAME, "r");
    if (!f) {
    ...
    ota_handle_t session = ota_session_open();
    ...
    while ((bytes_read = fread(buffer, 1, 1024, f)) > 0) {
        ...
        err = ota_write(session, buffer, bytes_read);
        if(err==false){
            break;
        }
        if (bytes_read < 1024) {
            ota_session_commit(session);
            app->updated=true;
            break;
        }
    }
...
void ui_update_prompt(app_state_t *app, mu_Context *ctx) {
    ...
    if(mu_begin_window(ctx, "Updater", mu_rect(0, 0, 720, 720 ))){
        mu_layout_row(ctx, 1, (int[]){-1}, 40);
        mu_text(ctx, "Press any button to start update");
        mu_end_window(ctx);
    }

    if (app->key_pressed && !app->updated) {
        read_update_file(app);
    }
}
