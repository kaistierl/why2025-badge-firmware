...
- **wolfSSH thread safety**: wolfSSH is built `SINGLE_THREADED` (no internal locking). `ssh_client_t` carries a `wolfssh_mutex` that serialises all outgoing wolfSSH calls (`wolfSSH_stream_send`, `wolfSSH_ChangeTerminalSize`). `wolfSSH_stream_read` intentionally does **not** hold the mutex — doing so deadlocks `read_input_thread` because `recv()` blocks indefinitely and `setsockopt` is not available as a shared symbol on BadgeVMS (a no-op stub is provided in `custom_io.c`). The residual concurrent send+receive race is documented in AUDIT_REPORT.md issue 2.2.
- **Terminal data ring buffer**: incoming SSH channel bytes flow through a 64 KB ring buffer (`term_buf` in `ssh_thread_manager_t`), not through the event queue. `read_peer_thread` writes into the ring buffer without blocking; the main thread drains it into libvterm each frame via `ssh_thread_drain_terminal_data()`. The event queue carries control signals only (connected / disconnected / auth / error). This keeps wolfSSH running at full speed regardless of rendering pace.
