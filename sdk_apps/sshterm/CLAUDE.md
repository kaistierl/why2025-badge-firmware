...
- **wolfSSH thread safety**: wolfSSH is built `SINGLE_THREADED` (no internal locking). `ssh_io_thread` is the **sole** wolfSSH caller after the handshake — it calls both `wolfSSH_stream_read` and `wolfSSH_stream_send`. No mutex is needed because there is only one caller. Raw socket bytes are delivered to wolfSSH via a cipher ring buffer (`ssh_client_t::cipher_rb_data`) filled by a dedicated `sock_reader_thread`. This eliminates the send+receive race that caused crashes under rapid input (see `WOLFSSH_THREADING_PLAN_INPUT.md` for the full design history).
- **Terminal data ring buffer**: incoming SSH channel bytes flow through a 64 KB ring buffer (`term_buf` in `ssh_thread_manager_t`), not through the event queue. `ssh_io_thread` writes into the ring buffer without blocking; the main thread drains it into libvterm each frame via `ssh_thread_drain_terminal_data()`. The event queue carries control signals only (connected / disconnected / auth / error). This keeps wolfSSH running at full speed regardless of rendering pace.
...
| Rendering bugs (cmatrix, box-drawing chars) | Open |
